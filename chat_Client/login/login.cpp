...
Loginw::Login(QWidget *parent): QStackedWidget(parent)
{
    ...
    connect(tcpSocket, &clientSock::signalStatus, this, &Login::sltTcpStatus);
    connect(tcpSocket, &clientSock::registerOk,
            this, &Login::signalRegisterOK);
...
void Login::sltTcpStatus(const quint8 &state)
{
    switch (state) {
    ...
    case LoginSuccess:{
        // 登陆验证成功
        disconnect(tcpSocket,&clientSock::signalStatus, this, &Login::sltTcpStatus);
        hideNotifyMsg();
        ...
        connect(globalFileSocket,&clientFileSock::signalFileRecvOk,
                this,&Login::sltFileRecvFinished);
//        myHelper::Sleep(1000);
...
void Login::sltFileRecvFinished(quint8,QString,int)
{
...
void Login::sltRegister(const QJsonValue &json)
{
//    tcpSocket->sltSendMessage(Register, json);
    tcpSocket->sendMsg(Register,json);
}

void Login::sltChangePwd(const QJsonValue &json)
{
//    tcpSocket->sltSendMessage(ChangePasswd,json);
    tcpSocket->sendMsg(ChangePasswd,json);
}

void Login::writeOffLineMsgToDatabase(const QJsonValue &dataVal)
{
...
void Login::showNotifyMsg(QString msg)
{
...
void Login::hideNotifyMsg()
{
    this->setFixedSize(430, 330);
    m_notifyMsg->hide();
}

void Login::showMainUI()
{
