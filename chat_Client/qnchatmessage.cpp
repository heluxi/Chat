...
//    te_font.setLetterSpacing(QFont::AbsoluteSpacing, 0);             //设置字间距为3像素 //设置字间距像素值
    ...
    m_leftPixmap = QPixmap(":/res/pic/Customer Copy.png");
    m_rightPixmap = QPixmap(":/res/pic/CustomerService.png");

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/res/pic/loading4.gif");
...
void QNChatMessage::setText(QString text, QString time, QSize allSize, QNChatMessage::User_Type userType)
{
    ...
    m_userType = userType;
    m_time = time;
    m_curTime = QDateTime::currentDateTime().fromSecsSinceEpoch(time.toInt()).toString("hh:mm");
    m_allSize = allSize;
    if(userType == User_Me) {
        if(!m_isSending) {
