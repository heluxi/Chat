...
{
// return QImage(QPixmap::grabWidget(this).toImage());//截取 QSLabel 对象的图像，然后将其转换为 QImage 对象并返回

//    QPixmap pixmap = QApplication::primaryScreen()->grabWindow(this->winId());
//    return QImage(pixmap.toImage());
    return grab().toImage();//于截取当前控件的图像，返回一个QPixmap对象
