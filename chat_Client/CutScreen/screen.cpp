...
Screen::Screen(QWidget *parent)
    : QWidget(parent),
      widthInfoRect(QRectF(0, 0, 0, 0)),//信息矩形
      control(NULL)
{
    showFullScreen();//将窗口部件全屏显示

    //截取全屏
    QScreen *scrPix = QGuiApplication::primaryScreen();//获取主屏幕
    pixmap = scrPix->grabWindow(0);
    //全局路径的初始化，一个全屏闭合回路顺时针绕着矩形的四个顶点依次连接起来
    ...
    labelimage->setWindowFlags(nType);//将以无边框窗口的形式显示，并且始终在其他窗口之上。
...
void Screen::paintEvent(QPaintEvent *e)//将上一个函数得到的全屏灰色图绘制在painter上，并添加宽高信息矩形和边角拖曳小正方形
{
    ...
    QPainter paint(this);
    paint.drawPixmap(0, 0, pixmap);//绘制全屏灰色
    //初始化画笔操作
